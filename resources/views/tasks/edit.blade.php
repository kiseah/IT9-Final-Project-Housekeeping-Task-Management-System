...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h4 class="fw-bold"><i class="bi bi-pencil"></i> Edit Task</h4>
    ...
    <form action="{{ route('tasks.update', $task->id) }}" method="POST">
            ...
            <div class="col-md-6">
                ...
                <select name="housekeeper_id"
                    ...
                    @foreach($housekeepers as $hk)
                        <option value="{{ $hk->id }}"
                            {{ old('housekeeper_id', $task->housekeeper_id) == $hk->id ? 'selected' : '' }}>
                            {{ $hk->full_name }}
                        </option>
                    @endforeach
                </select>
        ...
        <div class="mb-3">
            <label class="form-label fw-semibold">
                Linked Request <span class="text-muted">(optional)</span>
            </label>
            <select name="request_id"
                ...
                @foreach($requests as $req)
                    <option value="{{ $req->id }}"
                        {{ old('request_id', $task->request_id) == $req->id ? 'selected' : '' }}>
